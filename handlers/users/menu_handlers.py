...
async def list_menu(callback: types.CallbackQuery, lang, **kwargs):
    ...
    if lang == "ru":
        text = '''<b>Основное меню</b> Осмотрись, здесь ты найдёшь много полезной информации по замечательному проекту Axelar. (на данный момент информация добавляется)
    Разработчик бота: ollokio#7327
    '''
    else:
        text = '''<b>Main menu</b> Look around, here you will find a lot of useful information about the wonderful Axelar project. (information is currently being added)
